...
class ReplayBuffer:
    ...
    def add(
            ...
            ns: np.ndarray,
            d: Union[bool, np.ndarray],
            t: Union[bool, np.ndarray],
            log_prob: Optional[Union[float, np.ndarray]] = None,
    ) -> None:
        """
...
class VectorReplayBuffer(ReplayBuffer):
    def __init__(self, state_dim: Union[int, Tuple[int, ...]], action_dim: int, max_size: int = 10 ** 6,
        ...
        self.max_size = max_size#max(max_size // num_envs, 1)
        ...
        self.s = np.empty(self.s_size, dtype=np.float32)
        self.a = np.empty(self.a_size, dtype=np.float32)
        self.r = np.empty((self.max_size, num_envs), dtype=np.float32)
        self.ns = np.empty(self.s_size, dtype=np.float32)
        self.d = np.empty((self.max_size, num_envs,), dtype=np.float32)
        self.t = np.empty((self.max_size, num_envs,), dtype=np.float32)

        if save_log_prob:
            self.log_prob = np.empty((self.max_size, num_envs,), dtype=np.float32)
    ...
    def delete(self):
        self.s = np.empty(self.s_size, dtype=np.float32)
        self.a = np.empty(self.a_size, dtype=np.float32)
        self.r = np.empty((self.max_size, self.num_envs), dtype=np.float32)
        self.ns = np.empty(self.s_size, dtype=np.float32)
        self.d = np.empty((self.max_size, self.num_envs), dtype=np.float32)
        self.t = np.empty((self.max_size, self.num_envs), dtype=np.float32)

        if self.save_log_prob:
            self.log_prob = np.empty((self.max_size, self.num_envs), dtype=np.float32)
    ...
    def add(self,
        ...
        a = a.reshape((self.num_envs, self.action_dim))
